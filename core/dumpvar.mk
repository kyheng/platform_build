...
ifeq ($(TARGET_ARCH),arm)
    ...
    ifneq ($(wildcard $(PWD)/prebuilts/gcc/$(HOST_PREBUILT_EXTRA_TAG)/arm/arm-eabi-$(TARGET_GCC_VERSION)/bin),)
        # this should be copied to HOST_OUT_EXECUTABLES instead
        ABP:=$(ABP):$(PWD)/prebuilts/gcc/$(HOST_PREBUILT_EXTRA_TAG)/arm/arm-eabi-$(TARGET_GCC_VERSION)/bin
    else
...
# Add the x86 toolchain bin dir if it actually exists
    ifneq ($(wildcard $(PWD)/prebuilts/gcc/$(HOST_PREBUILT_EXTRA_TAG)/x86/i686-linux-android-$(TARGET_GCC_VERSION)/bin),)
        # this should be copied to HOST_OUT_EXECUTABLES instead
        ABP:=$(ABP):$(PWD)/prebuilts/gcc/$(HOST_PREBUILT_EXTRA_TAG)/x86/i686-linux-android-$(TARGET_GCC_VERSION)/bin
    else
